...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("                 - --   For Educational purpose only, ASTUTE WILL NOT BE RESPONSIBLE FOR ANY MISUSE -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by Astute Prof (") + handleClr.Sprintf("@astute_prof") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
